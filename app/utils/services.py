...
def service_has_or_is_expected_to_send_over_x_notifications(service: Service, num_notifications):
    if sum(v or 0 for k, v in service.volumes_by_channel.items()) > num_notifications:
        return True
    ...
    estimated_usage_this_year = (usage_this_year / percentage_through_current_financial_year()) * 100
