(function(Modules) {
  ...
  Modules.SetAuthTypeForm = function() {

    this.start = function(setAuthTypeForm) {
      this.$form = $(setAuthTypeForm);
      ...
      this.states.filter(state => state.key === 'nothing-selected-hint').forEach(x => this.addSelectAllButton(x));
      ...
      this.$form.on('change', 'input[type=checkbox]', () => this.userCheckboxChanged());
    };

    this.addSelectAllButton = function(state) {
    ...
    this.selectionStatus = {
      'default': 'No team members selected',
      'selected': numSelected => {
        const getString = key => {
          if (numSelected[key] === 0) {
            ...
            return `${numSelected[key]} team members`;
          }
        };
    ...
    this.userCheckboxChanged = function() {
