...
class TemplateFolderForm {
  ...
  setupStateModifiers() {
    ...
    this.states.filter(state => state.setFocus).forEach(x => x.$el.setAttribute('tabindex', '0'));
  ...
  countSelectedCheckboxes() {
    ...
    const templates = Array.from(allSelected).filter(el => 
      el.parentElement && el.parentElement.querySelector('.template-list-template')
    ).length;

    const folders = Array.from(allSelected).filter(el => 
      el.parentElement && el.parentElement.querySelector('.template-list-folder')
    ).length;
  ...
  createNothingSelectedButtons() {
    ...
    if (!this.$singleNotificationChannel) {
  ...
  createItemsSelectedButtons() {
    ...
    $countElement.textContent = this.selectionStatus.selected(1);
