...
def single_reply_to_email_address(mocker):
    ...
    return mocker.patch('app.service_api_client.get_reply_to_email_addresses', side_effect=_get, autospec=True)
...
def get_default_reply_to_email_address(mocker):
    ...
    return mocker.patch('app.service_api_client.get_reply_to_email_address', side_effect=_get, autospec=True)
...
def mock_add_reply_to_email_address(mocker):
    ...
    return mocker.patch('app.service_api_client.add_reply_to_email_address', side_effect=_add_reply_to, autospec=True)
...
def mock_update_reply_to_email_address(mocker):
    ...
    return mocker.patch('app.service_api_client.update_reply_to_email_address', side_effect=_update_reply_to, autospec=True)
...
def multiple_letter_contact_blocks(mocker):
    ...
    return mocker.patch('app.service_api_client.get_letter_contacts', side_effect=_get, autospec=True)
...
def get_default_letter_contact_block(mocker):
    ...
    return mocker.patch('app.service_api_client.get_letter_contact', side_effect=_get, autospec=True)
...
def mock_add_letter_contact(mocker):
    ...
    return mocker.patch('app.service_api_client.add_letter_contact', side_effect=_add_letter_contact, autospec=True)
...
def mock_update_letter_contact(mocker):
    ...
    return mocker.patch('app.service_api_client.update_letter_contact', side_effect=_update_letter_contact, autospec=True)
...
def multiple_sms_senders(mocker):
    ...
    return mocker.patch('app.service_api_client.get_sms_senders', side_effect=_get, autospec=True)
...
def get_default_sms_sender(mocker):
    ...
    return mocker.patch('app.service_api_client.get_sms_sender', side_effect=_get, autospec=True)
...
def mock_add_sms_sender(mocker):
    ...
    return mocker.patch('app.service_api_client.add_sms_sender', side_effect=_add_sms_sender, autospec=True)
...
def mock_update_sms_sender(mocker):
    ...
    return mocker.patch('app.service_api_client.update_sms_sender', side_effect=_update_sms_sender, autospec=True)
...
def multiple_available_inbound_numbers(mocker):
    ...
    return mocker.patch('app.inbound_number_client.get_available_inbound_sms_numbers', side_effect=_get, autospec=True)
...
def mock_get_service(mocker, api_user_active):
    ...
    return mocker.patch('app.service_api_client.get_service', side_effect=_get, autospec=True)
...
def mock_get_service_statistics(mocker, api_user_active):
    ...
    return mocker.patch('app.service_api_client.get_service_statistics', side_effect=_get, autospec=True)
...
def mock_get_detailed_services(mocker, fake_uuid):
    ...
    return mocker.patch('app.service_api_client.get_services', return_value=services, autospec=True)


@pytest.fixture(scope='function')
def mock_service_name_is_not_unique(mocker):
    return mocker.patch('app.service_api_client.is_service_name_unique', return_value=False, autospec=True)


@pytest.fixture(scope='function')
def mock_service_name_is_unique(mocker):
    return mocker.patch('app.service_api_client.is_service_name_unique', return_value=True, autospec=True)
...
def mock_create_service(mocker):
    ...
    return mocker.patch(
        'app.service_api_client.create_service', side_effect=_create, autospec=True)
...
def mock_get_services(mocker, active_user_with_permissions):
    ...
    return mocker.patch(
        'app.service_api_client.get_services', side_effect=_get_services, autospec=True)
...
def mock_get_service_email_template(mocker):
    ...
    return mocker.patch(
        'app.service_api_client.get_service_template', side_effect=_get, autospec=True)
...
def mock_create_service_template(mocker, fake_uuid):
    ...
    return mocker.patch(
        ...
        side_effect=_create, autospec=True)
...
def mock_get_service_templates_with_only_one_template(mocker):
    ...
    return mocker.patch(
        'app.service_api_client.get_service_templates',
        side_effect=_get, autospec=True)
...
def mock_delete_service_template(mocker):
    ...
    return mocker.patch(
        'app.service_api_client.delete_service_template', side_effect=_delete, autospec=True)
...
def mock_register_user(mocker, api_user_pending):
    ...
    return mocker.patch('app.user_api_client.register_user', side_effect=_register, autospec=True)
...
def mock_get_non_govuser(mocker, api_user_active):
    ...
    return mocker.patch(
        'app.user_api_client.get_user', side_effect=_get_user, autospec=True)
...
def mock_get_user_by_email_not_found(mocker, api_user_active):
    ...
    return mocker.patch(
        'app.user_api_client.get_user_by_email',
        side_effect=_get_user, autospec=True)
...
def mock_verify_password(mocker):
    ...
    return mocker.patch(
        ...
        side_effect=_verify_password, autospec=True)
...
def mock_update_user_password(mocker, api_user_active):
    ...
    return mocker.patch('app.user_api_client.update_password', side_effect=_update, autospec=True)
...
def mock_update_user_attribute(mocker, api_user_active):
    ...
    return mocker.patch('app.user_api_client.update_user_attribute', side_effect=_update, autospec=True)
...
def mock_activate_user(mocker, api_user_active):
    ...
    return mocker.patch('app.user_api_client.activate_user', side_effect=_activate, autospec=True)


@pytest.fixture(scope='function')
def mock_email_is_not_already_in_use(mocker):
    return mocker.patch('app.user_api_client.get_user_by_email_or_none', return_value=None, autospec=True)
...
def mock_revoke_api_key(mocker):
    ...
    return mocker.patch(
        ...
        side_effect=_revoke, autospec=True)
...
def mock_get_api_keys(mocker, fake_uuid):
    ...
    return mocker.patch('app.api_key_api_client.get_api_keys', side_effect=_get_keys, autospec=True)
...
def mock_send_verify_code(mocker):
    return mocker.patch('app.user_api_client.send_verify_code', autospec=True)


@pytest.fixture(scope='function')
def mock_send_verify_email(mocker):
    return mocker.patch('app.user_api_client.send_verify_email', autospec=True)
...
def mock_check_verify_code(mocker):
    ...
    return mocker.patch(
        'app.user_api_client.check_verify_code',
        side_effect=_verify, autospec=True)
...
def mock_create_job(mocker, api_user_active):
    ...
    return mocker.patch('app.job_api_client.create_job', side_effect=_create, autospec=True)
...
def mock_get_job(mocker, api_user_active):
    ...
    return mocker.patch('app.job_api_client.get_job', side_effect=_get_job, autospec=True)
...
def mock_has_jobs(mocker):
    mocker.patch('app.job_api_client.has_jobs', return_value=True, autospec=True)


@pytest.fixture(scope='function')
def mock_has_no_jobs(mocker):
    mocker.patch('app.job_api_client.has_jobs', return_value=False, autospec=True)
...
def mock_get_jobs(mocker, api_user_active):
    ...
    return mocker.patch('app.job_api_client.get_jobs', side_effect=_get_jobs, autospec=True)
...
def mock_get_uploads(mocker, api_user_active):
    ...
    return mocker.patch('app.models.job.PaginatedUploads.client_method', side_effect=_get_uploads, autospec=True)
...
def mock_has_permissions(mocker):
    ...
    return mocker.patch(
        ...
        side_effect=_has_permission, autospec=True)
...
def mock_get_users_by_service(mocker):
    ...
    return mocker.patch('app.models.user.Users.client_method', side_effect=_get_users_for_service, autospec=True)
...
def mock_s3_upload(mocker):
    ...
    return mocker.patch('app.main.views.send.s3upload', side_effect=_upload, autospec=True)
...
def mock_s3_download(mocker):
    ...
    return mocker.patch('app.main.views.send.s3download', side_effect=_download, autospec=True)


@pytest.fixture(scope='function')
def mock_s3_set_metadata(mocker):
    return mocker.patch('app.main.views.send.set_metadata_on_csv_upload', autospec=True,)
...
def mock_create_invite(mocker, sample_invite):
    ...
    return mocker.patch('app.invite_api_client.create_invite', side_effect=_create_invite, autospec=True)


@pytest.fixture(scope='function')
def mock_get_invites_for_service(mocker, service_one, sample_invite):
    def _get_invites(self, service_id):
        data = []
        for i in range(0, 5):
    ...
    return mocker.patch('app.models.user.InvitedUsers.client_method', side_effect=_get_invites, autospec=True)
...
def mock_check_invite_token(mocker, sample_invite):
    ...
    return mocker.patch('app.invite_api_client.check_token', side_effect=_check_token, autospec=True)
...
def mock_accept_invite(mocker, sample_invite):
    ...
    return mocker.patch('app.invite_api_client.accept_invite', side_effect=_accept, autospec=True)
...
def mock_add_user_to_service(mocker, service_one, api_user_active):
    ...
    return mocker.patch('app.user_api_client.add_user_to_service', side_effect=_add_user, autospec=True)


@pytest.fixture(scope='function')
def mock_set_user_permissions(mocker):
    return mocker.patch('app.user_api_client.set_user_permissions', return_value=None, autospec=True)


@pytest.fixture(scope='function')
def mock_remove_user_from_service(mocker):
    return mocker.patch('app.service_api_client.remove_user_from_service', return_value=None, autospec=True)
...
def mock_get_template_statistics(mocker, service_one, fake_uuid):
    ...
    return mocker.patch(
        'app.template_statistics_client.get_template_statistics_for_service', side_effect=_get_stats, autospec=True)
...
def mock_get_usage(mocker, service_one, fake_uuid):
    ...
    return mocker.patch(
        'app.billing_api_client.get_service_usage', side_effect=_get_usage, autospec=True)
...
def mock_get_billable_units(mocker):
    ...
    return mocker.patch(
        'app.billing_api_client.get_billable_units', side_effect=_get_usage, autospec=True)
...
def mock_events(mocker):
    ...
    return mocker.patch('app.events_api_client.create_event', side_effect=_create_event, autospec=True)


@pytest.fixture(scope='function')
def mock_send_already_registered_email(mocker):
    return mocker.patch('app.user_api_client.send_already_registered_email', autospec=True)
...
def mock_get_all_letter_branding(mocker):
    ...
    return mocker.patch(
        'app.letter_branding_client.get_all_letter_branding', side_effect=_get_letter_branding, autospec=True,
    )
...
def mock_get_letter_branding_by_id(mocker):
    ...
    return mocker.patch(
        'app.letter_branding_client.get_letter_branding', side_effect=_get_branding_by_id, autospec=True,
    )
...
def mock_no_email_branding(mocker):
    ...
    return mocker.patch(
        'app.email_branding_client.get_all_email_branding', side_effect=_get_email_branding, autospec=True,
    )
...
def mock_get_email_branding(mocker, fake_uuid):
    ...
    return mocker.patch(
        'app.email_branding_client.get_email_branding', side_effect=_get_email_branding, autospec=True,
    )
...
def mock_create_email_branding(mocker):
    ...
    return mocker.patch(
        'app.email_branding_client.create_email_branding', side_effect=_create_email_branding, autospec=True,
    )
...
def mock_update_email_branding(mocker):
    ...
    return mocker.patch(
        'app.email_branding_client.update_email_branding', side_effect=_update_email_branding, autospec=True,
    )
...
def mock_get_guest_list(mocker):
    ...
    return mocker.patch(
        'app.service_api_client.get_guest_list', side_effect=_get_guest_list, autospec=True,
    )
...
def mock_reset_failed_login_count(mocker):
    return mocker.patch('app.user_api_client.reset_failed_login_count', autospec=True)
...
def mock_create_service_data_retention(mocker):
    return mocker.patch(
        'app.service_api_client.create_service_data_retention',
        autospec=True)
...
def mock_update_service_data_retention(mocker):
    return mocker.patch(
        'app.service_api_client.update_service_data_retention',
        autospec=True)
...
def mock_get_valid_service_inbound_api(mocker):
    ...
    return mocker.patch('app.service_api_client.get_service_inbound_api', side_effect=_get, autospec=True)
...
def mock_get_valid_service_callback_api(mocker):
    ...
    return mocker.patch('app.service_api_client.get_service_callback_api', side_effect=_get, autospec=True)
...
def mock_create_service_inbound_api(mocker):
    ...
    return mocker.patch('app.service_api_client.create_service_inbound_api', side_effect=_create_service_inbound_api, autospec=True)
...
def mock_update_service_inbound_api(mocker):
    ...
    return mocker.patch('app.service_api_client.update_service_inbound_api', side_effect=_update_service_inbound_api, autospec=True)
...
def mock_create_service_callback_api(mocker):
    ...
    return mocker.patch('app.service_api_client.create_service_callback_api', side_effect=_create_service_callback_api, autospec=True)
...
def mock_update_service_callback_api(mocker):
    ...
    return mocker.patch('app.service_api_client.update_service_callback_api', side_effect=_update_service_callback_api, autospec=True)
...
def mock_get_organisations_with_unusual_domains(mocker):
    ...
    return mocker.patch('app.organisations_client.get_organisations', side_effect=_get_organisations, autospec=True)
...
def mock_get_organisation(mocker):
    ...
    return mocker.patch('app.organisations_client.get_organisation', side_effect=_get_organisation, autospec=True)
...
def mock_get_invited_users_for_organisation(mocker, sample_org_invite):
    def _get_invited_invited_users_for_organisation(self, org_id):
        return [
            sample_org_invite
        ]
...
def mock_get_invites_for_organisation(mocker, sample_org_invite):
    def _get_org_invites(self, org_id):
        data = []
        for i in range(0, 5):
    ...
    return mocker.patch('app.models.user.OrganisationInvitedUsers.client_method', side_effect=_get_org_invites, autospec=True)
...
def mock_check_org_invite_token(mocker, sample_org_invite):
    ...
    return mocker.patch('app.org_invite_api_client.check_token', side_effect=_check_org_token, autospec=True)
...
def mock_check_org_accepted_invite_token(mocker, sample_org_invite):
    ...
    return mocker.patch('app.org_invite_api_client.check_token', return_value=sample_org_invite, autospec=True)
...
def mock_accept_org_invite(mocker, sample_org_invite):
    ...
    return mocker.patch('app.org_invite_api_client.accept_invite', side_effect=_accept, autospec=True)
...
def mock_add_user_to_organisation(mocker, organisation_one, api_user_active):
    ...
    return mocker.patch('app.user_api_client.add_user_to_organisation', side_effect=_add_user, autospec=True)


@pytest.fixture(scope='function')
def mock_organisation_name_is_not_unique(mocker):
    return mocker.patch('app.organisations_client.is_organisation_name_unique', return_value=False, autospec=True)


@pytest.fixture(scope='function')
def mock_organisation_name_is_unique(mocker):
    return mocker.patch('app.organisations_client.is_organisation_name_unique', return_value=True, autospec=True)
...
def mock_update_organisation(mocker):
    ...
    return mocker.patch('app.organisations_client.update_organisation', side_effect=_update_org, autospec=True)
...
def mock_create_event(mocker):
    ...
    return mocker.patch('app.events_api_client.create_event', side_effect=_add_event, autospec=True)
...
def mock_get_template_folders(mocker):
    return mocker.patch('app.template_folder_api_client.get_template_folders', return_value=[], autospec=True)


@pytest.fixture
def mock_move_to_template_folder(mocker):
    return mocker.patch('app.template_folder_api_client.move_to_folder', autospec=True,)


@pytest.fixture
def mock_create_template_folder(mocker):
    return mocker.patch('app.template_folder_api_client.create_template_folder', return_value=sample_uuid(), autospec=True)
...
def mock_template_preview(mocker):
    ...
    mocker.patch('app.template_previews.TemplatePreview.from_database_object', return_value=example_response, autospec=True)
    mocker.patch('app.template_previews.TemplatePreview.from_valid_pdf_file', return_value=example_response, autospec=True)
    mocker.patch('app.template_previews.TemplatePreview.from_invalid_pdf_file', return_value=example_response, autospec=True)
    mocker.patch('app.template_previews.TemplatePreview.from_example_template', return_value=example_response, autospec=True)
    mocker.patch('app.template_previews.TemplatePreview.from_utils_template', return_value=example_response, autospec=True)
