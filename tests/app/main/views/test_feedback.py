...
def test_support_cannot_sign_in(client_request):
    ...
    assert radios[1]["value"] == "phone-number-changed"
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("phone-number-changed", "main.support_phone_number_changed", {}),
...
def test_support_phone_number_changed(client_request):
    client_request.logout()
    page = client_request.get("main.support_phone_number_changed")
    assert normalize_spaces(page.select_one("h1").text) == "If your phone number has changed"
    assert page.select_one(".govuk-back-link")["href"] == url_for("main.support_cannot_sign_in")
