...
@pytest.fixture
def mock_get_hierarchy_of_folders(mock_get_template_folders, active_user_with_permissions):
    mock_get_template_folders.return_value = [
        {"name": "Invisible folder", "id": str(uuid.uuid4()), "parent_id": None, "users_with_permission": []},
...
def test_template_list_yields_folders_visible_to_user(
    ...
    result_folder_names = tuple(
        result.name for result in UserTemplateList(service=service, user=user) if result.is_folder
    )
...
def test_template_list_yields_all_folders_without_user(
    ...
    result_folder_names = tuple(result.name for result in TemplateList(service=service) if result.is_folder)
