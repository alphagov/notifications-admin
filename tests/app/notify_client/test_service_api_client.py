...
def test_client_posts_archived_true_when_deleting_template(mocker):
    ...
    assert_mock_has_any_call_with_first_n_args(
        mock_redis_delete_by_pattern, f"service-{SERVICE_ONE_ID}-template-{FAKE_TEMPLATE_ID}*"
    )
...
def test_deletes_service_cache(
    ...
    assert_mock_has_any_call_with_first_n_args(mock_redis_delete, f"service-{SERVICE_ONE_ID}")
...
def test_deletes_caches_when_modifying_templates(
    ...
    for arg in expected_cache_deletes:
        assert_mock_has_any_call_with_first_n_args(mock_redis_delete, arg)

    for arg in expected_cache_deletes_by_pattern:
        assert_mock_has_any_call_with_first_n_args(mock_redis_delete_by_pattern, arg)
...
def test_deletes_cached_users_when_archiving_service(mocker, mock_get_service_templates):
    ...
    assert_mock_has_any_call_with_first_n_args(mock_redis_delete, "user-my-user-id1", "user-my-user-id2")
    assert_mock_has_any_call_with_first_n_args(mock_redis_delete_by_pattern, f"service-{SERVICE_ONE_ID}-template*")
