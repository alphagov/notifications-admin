...
def test_active_service_can_be_modified(app, method, user, service):
    api_client = NotifyAdminAPIClient()

    with app.test_request_context() as request_context, app.test_client() as client:
        client.login(user)
...
def test_inactive_service_cannot_be_modified_by_normal_user(app, api_user_active, method):
    api_client = NotifyAdminAPIClient()
...
def test_inactive_service_can_be_modified_by_platform_admin(app, platform_admin_user, method):
    api_client = NotifyAdminAPIClient()
...
def test_generate_headers_sets_standard_headers(app):
    api_client = NotifyAdminAPIClient()
    with set_config(app, 'ROUTE_SECRET_KEY_1', 'proxy-secret'):
...
def test_generate_headers_sets_request_id_if_in_request_context(app):
    api_client = NotifyAdminAPIClient()
    api_client.init_app(app)
