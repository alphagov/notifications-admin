...
@pytest.mark.parametrize("delivery_statistics", [None, [{}], [{"emails_requested": 0}, {"emails_requested": 0}]])
...
@pytest.mark.parametrize(
    "emails_failed,emails_requested,expected_failure_rate", [(0, 0, "0"), (0, 1, "0.0"), (1, 3, "33.3")]
)
def test_add_rates_sets_email_failure_rate(emails_failed, emails_requested, expected_failure_rate):
    resp = add_rates_to(
        {"emails_failed": emails_failed, "emails_requested": emails_requested, "sms_failed": 0, "sms_requested": 0}
    )

    assert resp["emails_failure_rate"] == expected_failure_rate


@pytest.mark.parametrize("sms_failed,sms_requested,expected_failure_rate", [(0, 0, "0"), (0, 1, "0.0"), (1, 3, "33.3")])
def test_add_rates_sets_sms_failure_rate(sms_failed, sms_requested, expected_failure_rate):
    resp = add_rates_to(
        {"emails_failed": 0, "emails_requested": 0, "sms_failed": sms_failed, "sms_requested": sms_requested}
    )
