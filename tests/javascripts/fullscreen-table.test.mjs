...
describe('FullscreenTable with govuk-frontend table', () => {
  let screenMock;
  let container;
  let caption;
  let tableFrame;
  ...
  beforeEach(() => {
      ...
      headings.forEach((heading, idx) => {
        if (idx === 0) {
          result += `<th scope="col" class="govuk-table__header">Date</th>
                        <span class="govuk-visually-hidden">Row in file</span><span aria-hidden="true">${heading}</span>
                      </th>`;
        } else {
    ...
    const rowCells = (cells) => {
      ...
      Object.keys(cells).forEach((key, idx) => {
        if (idx === 0) {
          result += `<td class="govuk-table__cell govuk-!-font-size-16">
                      ${key}
                    </td>`;
        } else {
          result += `<td class="govuk-table__cell govuk-!-font-size-16">
